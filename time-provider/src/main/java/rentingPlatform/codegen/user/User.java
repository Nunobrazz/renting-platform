...
public final class User extends Template {
  public static final Identifier TEMPLATE_ID = new Identifier("3ab0b0842cdffab8c416d02894ad44bcc570eeb048f996af3c2257fb08640ba3", "User", "User");
  ...
  public static final Choice<User, SendMessage, Message.ContractId> CHOICE_SendMessage = 
      Choice.create("SendMessage", value$ -> value$.toValue(), value$ -> SendMessage.valueDecoder()
        .decode(value$), value$ ->
        new Message.ContractId(value$.asContractId().orElseThrow(() -> new IllegalArgumentException("Expected value$ to be of type com.daml.ledger.javaapi.data.ContractId")).getValue()));

  public static final Choice<User, rentingPlatform.codegen.da.internal.template.Archive, Unit> CHOICE_Archive = 
  ...
  public static final ContractCompanion.WithKey<Contract, ContractId, User, String> COMPANION = 
      new ContractCompanion.WithKey<>("rentingPlatform.codegen.user.User", TEMPLATE_ID,
        ContractId::new, v -> User.templateValueDecoder().decode(v), User::fromJson, Contract::new,
        List.of(CHOICE_Follow, CHOICE_SendMessage, CHOICE_Archive),
  ...
  /**
   * @deprecated since Daml 2.3.0; use {@code byKey(key).exerciseSendMessage} instead
   */
  @Deprecated
  public static Update<Exercised<Message.ContractId>> exerciseByKeySendMessage(String key,
      SendMessage arg) {
    return byKey(key).exerciseSendMessage(arg);
  }

  /**
   * @deprecated since Daml 2.3.0; use {@code byKey(key).exerciseSendMessage(text, sender)} instead
   */
  @Deprecated
  public static Update<Exercised<Message.ContractId>> exerciseByKeySendMessage(String key,
      String text, String sender) {
    return byKey(key).exerciseSendMessage(text, sender);
  }
  ...
  /**
   * @deprecated since Daml 2.3.0; use {@code createAnd().exerciseSendMessage} instead
   */
  @Deprecated
  public Update<Exercised<Message.ContractId>> createAndExerciseSendMessage(SendMessage arg) {
    return createAnd().exerciseSendMessage(arg);
  }
   ...
   */
  @Deprecated
  public Update<Exercised<Message.ContractId>> createAndExerciseSendMessage(String text,
      String sender) {
    return createAndExerciseSendMessage(new SendMessage(text, sender));
  }
  ...
  public String toString() {
    return String.format("rentingPlatform.codegen.user.User(%s, %s)", this.username,
        this.following);
  }
  ...
  public interface Exercises<Cmd> extends com.daml.ledger.javaapi.data.codegen.Exercises.Archive<Cmd> {
    ...
    default Update<Exercised<Message.ContractId>> exerciseSendMessage(SendMessage arg) {
      return makeExerciseCmd(CHOICE_SendMessage, arg);
    }

    default Update<Exercised<Message.ContractId>> exerciseSendMessage(String text, String sender) {
      return exerciseSendMessage(new SendMessage(text, sender));
    }
